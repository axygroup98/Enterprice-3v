...
// ─── ERP Detail Drawer ────────────────────────────────────────────────
function ErpDetailDrawer({ product, listings, onClose }: { product: ErpProduct; listings: MarketplaceListing[]; onClose: () => void }) {
    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl z-50 overflow-y-auto">
        ...
        <div className="px-6 py-4 space-y-6">
          ...
          {linked.length > 0 && (
            <div>
              <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3 flex items-center gap-1.5"><Link2 className="h-4 w-4" /> Anúncios vinculados ({linked.length})</h3>
              <div className="space-y-2">
                {linked.map((l) => (
...
export function Monitor() {
  ...
  return (
    <div className="space-y-6">
      ...
      {selectedErp && <ErpDetailDrawer product={selectedErp} listings={listings} onClose={() => setSelectedErp(null)} />}
